...
class DepTreeBuilder(AbstractDepTreeBuilder):
    ...
    def create_venv(self, path: str = PATH, force_recreate: bool = False) -> str:
        ...
        if force_recreate and self.venv_exists(path):
            self.delete_venv(path)
    ...
    def delete_venv(self, path: str = PATH) -> None:
    ...
    def get_tree_json(self, venv_bin: str) -> List[Dict]:
        ...
        try:
            ...
            logger.critical(
                "pipdeptree failed with exit code %s", exc.returncode)
    ...
    def print_subtree(self, graph: Dict[str, List[str]], pkg: str, indent: int = 0,
        ...
        for dep in graph.get(pkg, []):
            self.print_subtree(graph, dep, indent + 4, visited)
...
def main() -> None:
    ...
    try:
        ...
        graph = builder.build_map(tree_json)
        for pkg in graph:
            print(pkg)
